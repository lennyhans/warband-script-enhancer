...
void ServerSetMaxNumPlayers(WSENetworkOperationsContext *context)
{
	...
	if (!data_basic_game->basic_game.is_multiplayer())
		return;
	...
	data_string_manager->multiplayer_data.max_num_players = max_num_players;
	data_string_manager->multiplayer_data.max_num_private_players = max_num_private_players;
}
...
void ServerSetPasswordAdmin(WSENetworkOperationsContext *context)
{
	...
	if (!data_basic_game->basic_game.is_multiplayer() || !data_basic_game->basic_game.is_safe_string(str))
		return;

	data_string_manager->multiplayer_data.server_password_admin = str;
}
...
void ServerSetPasswordPrivate(WSENetworkOperationsContext *context)
{
	...
	data_string_manager->multiplayer_data.server_password_private = str;
}

int ServerMapRotationGetNumEntries(WSENetworkOperationsContext *context)
{
#if defined WARBAND_DEDICATED
	return data_string_manager->multiplayer_data.maps_list.size();
#else
	return -1;
#endif
}
...
{
#if defined WARBAND_DEDICATED
	return data_string_manager->multiplayer_data.cur_map_index;
#else
	return -1;
#endif
}
...
void ServerMapRotationSetIndex(WSENetworkOperationsContext *context)
{
	...
	if (index < 0 || index >= data_string_manager->multiplayer_data.maps_list.size())
		return;
	
	data_string_manager->multiplayer_data.cur_map_index = index;
#endif
}
...
int ServerMapRotationGetMap(WSENetworkOperationsContext *context)
{
	...
	return data_string_manager->multiplayer_data.maps_list[index];
#else
	return -1;
#endif
}
...
void ServerMapRotationAddMap(WSENetworkOperationsContext *context)
{
	...
	if (context->HasMoreOperands())
	{
		...
		context->ExtractValue(index);
		data_string_manager->multiplayer_data.maps_list.insert(index, site_no);
	}
	else
	{
		data_string_manager->multiplayer_data.maps_list.push_back(site_no);
	}
#endif
}
...
{
#if defined WARBAND_DEDICATED
	if (data_string_manager->multiplayer_data.maps_list.size() == 1)
		return;
	...
	if (context->HasMoreOperands())
	{
		...
		context->ExtractValue(index);
		data_string_manager->multiplayer_data.maps_list.remove_at(index);
	}
	else
	{
		data_string_manager->multiplayer_data.maps_list.pop_back();
	}
#endif
}
...
int GetServerOptionAtConnect(WSENetworkOperationsContext *context)
{
	...
	if (!data_basic_game->basic_game.is_client())
		return -1;

	const char *connected_server_ip = inet_ntoa(data_string_manager->multiplayer_data.server_address.ip);
	int connected_server_port = data_string_manager->multiplayer_data.server_address.port;

	for (int i = 0; i < data_string_manager->multiplayer_data.server_list.size(); ++i)
	{
		wb::multiplayer_server *server = &data_string_manager->multiplayer_data.server_list[i];

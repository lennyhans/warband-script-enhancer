...
__int64 operation::get_operand_value(__int64 *locals, const int &operand_no, int &operand_type)
{
	...
	case 0:
		return value;
	case 1:
		return data_basic_game->basic_game.registers[(int)value];
	case 2:
		return data_basic_game->basic_game.global_variables.get((int)value);
	case 17:
		return locals[(int)value];
	default:
		return value;
...
void operation::set_return_value(__int64 *locals, const __int64 &value)
{
	...
	{
	case 1:
		data_basic_game->basic_game.registers[(int)this->operands[0]] = value;
		break;
	case 2:
		data_basic_game->basic_game.global_variables.set((int)this->operands[0], value);
		break;
	case 17:
		locals[(int)this->operands[0]] = value;
		break;
